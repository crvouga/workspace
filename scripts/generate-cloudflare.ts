...
function generateIndexTs(): string {
  ...
  lines.push("");
  lines.push(`const RECOVERY_INSTANCE_SUFFIX = "recovery-v1";`);
  lines.push(``);
  lines.push(`function isRecoverableStartupFailure(bodyText: string): boolean {`);
  lines.push(`  const normalized = bodyText.toLowerCase();`);
  lines.push(`  return (` );
  lines.push(`    normalized.includes("failed to start container") ||`);
  lines.push(`    normalized.includes("the container is not running") ||`);
  lines.push(`    normalized.includes("the operation was aborted") ||`);
  lines.push(`    normalized.includes("there is no container instance available at this time")`);
  lines.push(`  );`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function fetchContainerResponse(`);
  lines.push(`  ns: DurableObjectNamespace<PortfolioContainerBase>,`);
  lines.push(`  request: Request,`);
  lines.push(`  instanceId: string,`);
  lines.push(`): Promise<Response> {`);
  lines.push(`  try {`);
  lines.push(`    const instance = getContainer(ns, instanceId);`);
  lines.push(`    return await instance.fetch(request);`);
  lines.push(`  } catch (err) {`);
  lines.push(`    const message = err instanceof Error ? err.message : String(err);`);
  lines.push(`    return new Response(\`Failed to start container: \${message}\`, { status: 500 });`);
  lines.push(`  }`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function fetchWithStartupRecovery(`);
  lines.push(`  ns: DurableObjectNamespace<PortfolioContainerBase>,`);
  lines.push(`  request: Request,`);
  lines.push(`  hostname: string,`);
  lines.push(`): Promise<Response> {`);
  lines.push(`  const primaryResponse = await fetchContainerResponse(ns, request, hostname);`);
  lines.push(`  if (!["GET", "HEAD", "OPTIONS"].includes(request.method.toUpperCase())) {`);
  lines.push(`    return primaryResponse;`);
  lines.push(`  }`);
  lines.push(``);
  lines.push(`  if (primaryResponse.status < 500) {`);
  lines.push(`    return primaryResponse;`);
  lines.push(`  }`);
  lines.push(``);
  lines.push(`  const primaryBody = await primaryResponse.clone().text();`);
  lines.push(`  if (!isRecoverableStartupFailure(primaryBody)) {`);
  lines.push(`    return primaryResponse;`);
  lines.push(`  }`);
  lines.push(``);
  lines.push(`  const recoveryInstanceId = \`\${hostname}:\${RECOVERY_INSTANCE_SUFFIX}\`;`);
  lines.push(`  const recoveryResponse = await fetchContainerResponse(ns, request, recoveryInstanceId);`);
  lines.push(`  if (recoveryResponse.status < 500) {`);
  lines.push(`    return recoveryResponse;`);
  lines.push(`  }`);
  lines.push(``);
  lines.push(`  const recoveryBody = await recoveryResponse.clone().text();`);
  lines.push(`  if (!isRecoverableStartupFailure(recoveryBody)) {`);
  lines.push(`    return recoveryResponse;`);
  lines.push(`  }`);
  lines.push(``);
  lines.push(`  return primaryResponse;`);
  ...
  lines.push(`    }`);
  lines.push("");
  lines.push(`    const ns = env[bindingKey as ContainerBindingKey] as DurableObjectNamespace<PortfolioContainerBase>;`);
  lines.push(`    return fetchWithStartupRecovery(ns, request, hostname);`);
  lines.push(`  },`);
